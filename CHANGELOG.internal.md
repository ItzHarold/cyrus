...
### Changed
- **Repo routing never guesses a repository.** An issue from a team with no repository mapping now **refuses at intake** in client language ("we're not set up to work in this team's repository yet — the operator has been notified") and emits `[event:repo_unmapped]`, instead of silently routing to a "catch-all" repo (any repo with no routing config) or asking the client to pick one from a list. The catch-all routing priority is removed entirely — a repository is only ever selected through an explicit mapping (description tag, routing label, team key, or project key). When a team legitimately maps to **more than one** repository the route is ambiguous, so the agent asks once with a canonical picker of the matching repositories rather than silently taking the first. A reply that matches none of the offered repositories no longer starts work in the first-configured repo — it re-asks. Repository routing was already scoped strictly to the issue's own workspace (an issue can never resolve to another tenant's repo); that invariant, plus the two new ones, are now pinned by tests.

### Fixed
- **Tenant isolation: the cockpit's queue annotations never cross a tenant boundary.** With two clients' work queued together, a mirror's description named another tenant's issue as its blocker (found live: a FrontDoor mirror rendered as "waiting on" an Acme item). The queue rank was global and the "behind …" annotation accumulated a cross-tenant list. Now each mirror shows its place **within its own tenant's queue** ("#N of M"), a gated one gives only its own tenant's reason, and the "behind" annotation is retired. The single global marker that remains — "▶ Suggested next" — is relabelled a read-only suggestion across clients that gates nothing (the start path reads only same-tenant WIP and the per-workspace lane, never a rank). The client-content sweep gained a `cross-tenant-position` ban (cross-client/global-queue/foreign-blocker vocabulary) so the operator's board language can never reach a client surface, and an invariant test over random multi-tenant boards pins the suggestion as a pure function that leaves each tenant's own order and gates identical when other tenants are removed.
- **A newly-created cockpit mirror no longer reads "Agent didn't start — Retry".** Linear opens an agent session when a mirror is created; on parked work nothing runs on it, so Linear rendered it as a failed/retry state on work that was simply waiting to be delegated. The mirror's birth session now gets one honest closing line — "Parked — nothing running here; delegate to start." — posted as a response so the turn completes cleanly. The reviewer's delegation reuses the same session.
- **Delivery surface polish.** A delivery now shows the reviewer's full name ("Reviewed by …") rather than their short GitHub-style handle. And the preview and pull-request links appear once, in the delivery footer, instead of being repeated in the summary above it — the summary keeps the words ("open the preview, go to Overview, scroll to the bottom"), the footer keeps the links.
- **A client's session no longer shows the model's internal task plan.** Linear renders an agent session's plan as a numbered step list at the top of the thread; after plan publishing started working, a client session was rendering the model's raw internal task list there. Client sessions now show a fixed, code-maintained lifecycle plan — **Scope agreed → In development → Ready for your review → Merged** — whose step status is driven by the state machine at each transition, never by the model. Mirror (reviewer) sessions keep the detailed task plan. Regression-tested: no client session ever publishes the model plan, and the lifecycle steps pass the client-content sweep.
- **A WIP-gated mirror delegation no longer opens a second thread.** When a reviewer delegated or re-assigned a parked mirror whose client already had work in flight, the assignment-recovery path could mint a redundant second agent thread on the mirror instead of reusing the one it already had — observed live on CKP-25, where a WIP-refused delegation was followed 60s later by `assignment_session_recovered` opening a second thread. Recovery now reuses the mirror's birth/narration thread when no operator link exists yet (that link is registered only once work starts, downstream of the WIP gate), so a mirror gets exactly one thread, the refusal explanation lands on it, and the mirror stays Queued. Regression-tested end-to-end on both entry paths — delegate (`agentSessionCreated`) and assign (`recoverMissingSessionForAssignment`). ([#124](https://github.com/ItzHarold/cyrus/pull/124))
- **A `reject:` starts a new run for the hand-over guard**, so a client summary recorded before the rejection can no longer be re-held unchanged. Four deletions alongside: the dead post-reject guard, the rework confirmation's copy of the label-plus-note parser (now shared with the scope gate), a double scrape of pull-request URLs, and the waiting room's redundant reopen of an already-open room. ([#111](https://github.com/ItzHarold/cyrus/pull/111))
