...
**The client-surface sweep now covers it.** `client-content-policy.ts` gained the `cross-tenant-position` rule — cross-client/global-queue/foreign-blocker/queue-depth vocabulary ("across clients", "waiting on FRO-76", "N issues ahead", "global queue"). NOT redactable: on a client surface this copy is wrong at the source. The per-tenant client ack ("position #3 in your queue") and the one-issue-at-a-time promise match none of it. The cockpit mirror stays `OPERATOR_SIDE`-exempt from the static sweep — the operator's board is exactly where "Suggested next across clients" belongs — so the ban lands on the CLIENT surfaces only.
