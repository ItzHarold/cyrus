...
### TENANT ISOLATION — the workspace is the boundary (2026-09-02, PON-223 punch list)

**The invariant.** The tenant (workspace) is the isolation boundary. Gating, ranking, "behind/waiting" annotations, and every client-visible string reference **only items of the same tenant**. Capacity is per tenant (a lane per workspace). The **only** cross-tenant construct is the operator's suggested pick order — a read-only view over the per-tenant queues, labelled a suggestion, that **gates nothing**. Found live: three Acme issues and two FrontDoor issues queued together rendered `CKP-42 (FrontDoor)` as "waiting on" an Acme item — a cross-tenant dependency expressed in the annotation layer, and the exact shape this bans.

Most of the machinery already held this by construction — the audit's value was proving which half did and which did not:

- **Gates are per-tenant and were never the leak.** The WIP gate (`clientWorkInFlight`, keyed by client/workspace — `CockpitMirror.ts` skips any record whose `keyOf` ≠ the subject's) and the lane (`LaneManager`, keyed by `workspaceId`) only ever consult same-tenant state. `mayStartParkedWork` consults the WIP gate alone — never a rank, never the suggestion. Cross-tenant ordering **cannot** gate, because nothing on the start path reads it.
- **The annotation layer WAS the leak, and is where the fix lives.** `queueRank` was global and `behind` accumulated a cross-tenant `ahead[]` list — so a mirror's description named another tenant's issue as its blocker. Now: rank is computed **per tenant** (`perTenant` map keyed by `tenantWorkspaceId` in both derivation loops), the record carries `clientQueuePosition` (place within its OWN tenant's queue) and `tenantQueueTotal` (the M in "#N of M"), and `behind` is **retired — always written `undefined`**. `queueRank` survives only as an internal marker that drives `sortOrder`; it is **never rendered as a rank**.
- **`nextUp` stays the one global marker and is relabelled, not removed.** It renders as "▶ **Suggested next** across clients — a suggestion, not a queue; nothing waits on it." It is derived by `computeRoundRobinOrder` (pure — deterministic, non-mutating; see the invariant test), sets no state on any record, and gates nothing. Removing every other tenant leaves each tenant's own ranks and gates identical, because a tenant's rank is a function of its own items only.
- **`DESCRIPTION_VERSION` bumped 5 → 6** in the same commit as the render change, per the standing rule (existing mirrors keep the old body until it changes).

**The client-surface sweep now covers it.** `client-content-policy.ts` gained the `cross-tenant-position` rule — cross-client/global-queue/queue-depth vocabulary ("across clients", "another client's", "N issues ahead", "global queue"). NOT redactable: on a client surface this copy is wrong at the source. The same function REFUSES four operator→client text paths (delivery notes, cancels, reviewer questions, needs-info relays), so the pattern is narrowed to phrasing that is ALWAYS cross-tenant/global — it deliberately does **not** match a bare "waiting on `<ISSUE-ID>`", because an operator note may reference the client's OWN issue and refusing that would false-block a delivery (the identifier case is covered structurally — the cockpit no longer renders cross-tenant "behind" at all). The per-tenant client ack ("position #3 in your queue") and the one-issue-at-a-time promise match none of it. The cockpit mirror stays `OPERATOR_SIDE`-exempt from the static sweep — the operator's board is exactly where "Suggested next across clients" belongs — so the ban lands on the CLIENT surfaces only.

**The invariant is tested** (`tenant-isolation.test.ts`) against random multi-tenant boards: (a) the suggested order permutes only its own inputs (no foreign token); (b) `computeRoundRobinOrder` is pure (deterministic + non-mutating — a suggestion changes nothing); (c) removing every other tenant leaves a tenant's own order identical. Plus the `CockpitMirror` tests assert a two-tenant board renders "#N of M" per tenant, a gated mirror shows only its OWN tenant's reason, and no description names the other tenant or the word "behind".
