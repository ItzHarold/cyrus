...
### Safe config.json writes (PON-190 item 3)

`config.json` holds every tenant's Linear tokens and preview bypass credentials, and had **four** independent writers across **two processes**: `ConfigService.save`, `SelfAddRepoCommand`, `SelfAuthCommand` (all CLI) and `config-updater/handlers/cyrusConfig` (in the running service). Every one was a plain `writeFileSync` over a read-modify-write.

Two distinct failures needing two distinct protections, and conflating them is the trap: **torn file** (interleaved writes producing unparseable JSON) is fixed by temp+rename; **lost update** (A reads, B reads, A writes, B writes, A's tenant gone) is NOT — both writes are individually perfect and the second still erases the first. That needs a lock held across the read AND the write. `PersistenceManager.saveEdgeWorkerState` solved only the first, for the state file, with an in-process chain — which cannot help here because the racing writers are different processes.

`core/src/atomic-config.ts` → `updateConfigFile(path, mutate, {mode, indent})`: exclusive lockfile (`openSync` `'wx'`), read inside the lock, `mutate`, temp + chmod + rename, release in `finally`.

- **All four writers go through it.** A lock only some writers take is not a lock, it is false confidence — so `ConfigService.save` and the config-updater handler take it too even though they replace the whole file and have nothing to merge.
- **Stale locks (>10s) are broken.** A holder that dies between create and unlink would otherwise block every future write permanently — an outage needing shell access, which is worse than the race it guards.
- **Temp files are per-writer** (`<path>.<pid>.<n>.tmp`). A shared `<path>.tmp` is safe only while the lock is the sole entry point; `writeConfigAtomicUnlocked` is also exported, and two of those would write halves of each other's file and rename the result over config.json.
- **Unparseable existing content is rethrown, never overwritten.** Replacing it with fresh content would delete every tenant on the box because one byte was wrong.
- **Residual, stated:** `ConfigService.save` replaces the caller's whole config, so a caller that read the file long before calling it can still write back a stale copy. Closing that means making `save()` merge, which changes every caller. The lock makes the write atomic and mutually exclusive; it does not fix a stale read the caller already did.

Testing note: the CLI and config-updater suites mock `node:fs` partially, so they need `openSync`/`closeSync`/`unlinkSync`/`renameSync`/`statSync` added or the real writer cannot load. Prefer that over mocking `cyrus-core` — the assertions then still exercise the real writer. A missing file must be mocked as **throwing ENOENT**, not returning `""`: the latter is a parse failure, which the writer correctly refuses.
