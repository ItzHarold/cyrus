...
3. Post one comment with the DELIVERABLE-framed scope — the same text you recorded as client_scope — written entirely in terms of what the client receives. Structure it as: **Outcome** — what will exist and work when this is done, in the client's own terms; **You will receive** — where they will see it working and what they will get to merge; **Interpreted** — anything you had to interpret about the outcome itself, stated so they can correct it. No implementation detail anywhere in this comment: no file names, no code areas, no approach, no technical steps — that is what the operator note is for.
