...
2. Record your INTERNAL reading with the record_operator_note tool: implementation approach, the files and areas you expect to touch, risks, and every interpretation you made. Full technical detail — this goes to the operator only and is never shown on the issue. If the issue is genuinely ambiguous, resolve the ambiguity first with the AskUserQuestion tool, then record the note.
3. Post one comment with the DELIVERABLE-framed scope, written entirely in terms of what the client receives. Structure it as: **Outcome** — what will exist and work when this is done, in the client's own terms; **You will receive** — where they will see it working and what they will get to merge; **Interpreted** — anything you had to interpret about the outcome itself, stated so they can correct it. No implementation detail anywhere in this comment: no file names, no code areas, no approach, no technical steps — that is what the operator note is for.
4. Ask for confirmation with the AskUserQuestion tool — exactly one question, asking whether to proceed with the scope you posted, with these options in this order: "${SCOPE_APPROVE_LABEL}", "${SCOPE_REVISE_LABEL}", "${SCOPE_CANCEL_LABEL}". Use those labels exactly, and never use the label "${SCOPE_APPROVE_LABEL}" on any other question.
5. Act on the answer:
