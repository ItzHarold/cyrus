...
### Changed
- A claimed queued mirror keeps its rank and can be next up; a claim only takes in-verification and needs-info mirrors out of the order ([#117](https://github.com/ItzHarold/cyrus/pull/117))
- Cockpit mirrors now carry exactly one agent thread, created by the reviewer's delegation; re-delegating reuses it. The working order is explicit: one queued mirror carries the `next-up` label, every queued mirror shows its rank and what it is behind, and a gated one says why. Statuses are the state — no more state labels; the waiting room lives in Backlog and closes to Done ([#116](https://github.com/ItzHarold/cyrus/pull/116))
...
### Fixed
- The reviewer hand-off and merge close-out now show real timings — build time, queue wait, and total cycle time from go-ahead to merge — measured on our own clock rather than Linear's thread-age timer ([#123](https://github.com/ItzHarold/cyrus/pull/123))
- Building the work now happens only on the reviewer's delegated session, enforced by capability: a client's own thread can scope, ask and confirm but never edits or commits, and a mirror can't build until you delegate it. A change asked for on delivered work is offered back as a confirm rather than made directly ([#122](https://github.com/ItzHarold/cyrus/pull/122))
- Work is no longer started before its scope is approved: until the client approves, the session proposes rather than builds, so the queued item is genuinely queued and built only when you pick it up ([#121](https://github.com/ItzHarold/cyrus/pull/121))
- Delivered work can no longer be changed directly on the client's thread: a change request is restated and confirmed, then re-enters review as rework — the client never receives an unreviewed change ([#120](https://github.com/ItzHarold/cyrus/pull/120))
- When you ask a client a question about work that's already held for review, the answer now re-opens the review turn cleanly: you get a fresh hand-off and the item returns to In verification, instead of the session appearing to work forever ([#119](https://github.com/ItzHarold/cyrus/pull/119))
- A completed run now delivers even if it left a background task running: the final result is the completion signal, the task is stopped with the run, and the reviewer hand-off names what was stopped. Scheduled wakeups still hold the session open as before ([#118](https://github.com/ItzHarold/cyrus/pull/118))
- **Preview links no longer carry your access value.** Links now point at our own address and open the preview from there, so the value stays with us; regenerating it does not break links already sent, and links stop working when the issue is closed. ([#113](https://github.com/ItzHarold/cyrus/pull/113))
- **Preview links in a delivery no longer swallow the sentence's full stop.** A link written at the end of a sentence was rewritten with the punctuation inside it and opened a page that did not exist. ([#111](https://github.com/ItzHarold/cyrus/pull/111))
- **The delivery's signature names only the reviewer who claimed the work.** If nobody has, the line is left out rather than naming someone else. ([#111](https://github.com/ItzHarold/cyrus/pull/111))
- **Your merge is never closed out in silence.** If the message confirming your merge fails to post, it is retried and your issue stays open until it lands, and the reviewer is told. Previously the issue was completed without the message. ([#110](https://github.com/ItzHarold/cyrus/pull/110))
- **A change you ask for after delivery is now reviewed again before it comes back.** Previously the reworked version could complete without going through review, and releasing it answered "already delivered". The pull request also returns to draft while the change is being made, and your confirmation still counts if it arrives during a restart. ([#109](https://github.com/ItzHarold/cyrus/pull/109))
- **Stopping a run from the cockpit no longer closes the work as done.** A mirror run that is stopped, crashes, or is interrupted by a restart goes back to the queue with a note; delegating the mirror again picks it up on the same branch. Previously the mirror closed as delivered with the work half-built. ([#108](https://github.com/ItzHarold/cyrus/pull/108))
- **A plain instruction on a cockpit run no longer changes where the delivery goes.** After one iteration, a later rewrite request could regenerate on the client's own thread; it now stays on the cockpit. ([#108](https://github.com/ItzHarold/cyrus/pull/108))
- **Writing on your issue while we are working no longer restarts anything on your side.** You get a short acknowledgement, and your words go to the reviewer. Previously a message mid-work could start a second, unreviewed run on your own thread. ([#112](https://github.com/ItzHarold/cyrus/pull/112))
- **The agent's Linear credential no longer appears in the command line of the session process.** The tool-server configuration, which carries that credential, is now handed to the session as a private file that exists only while the session runs. ([#105](https://github.com/ItzHarold/cyrus/pull/105))
- **The cockpit's waiting list no longer keeps a stale row after it closes.** When the last waiting conversation was answered, the list closed but still showed that conversation as waiting; it now empties itself first. ([#104](https://github.com/ItzHarold/cyrus/pull/104))
- **The agent's checklist now shows in the session panel.** The step-by-step plan was sent in a shape the platform refuses, so it never rendered for anyone; it now does. ([#103](https://github.com/ItzHarold/cyrus/pull/103))
- **A clarifying question asked before a scope is proposed no longer vanishes from the operator's waiting list.** When the agent asks a client something before it can propose a scope, that conversation now shows up as waiting; previously it was invisible until the client answered. ([#101](https://github.com/ItzHarold/cyrus/pull/101))
- **A configuration reload can no longer drop a connected workspace.** The agent re-reads its configuration while running, and a workspace momentarily missing from that file was removed from the running agent — taking its access with it, so that client's work silently stopped with nothing reported. Workspaces are now merged rather than replaced: a change to one never removes another. Deliberately disconnecting a workspace is unaffected.
- **Connecting a repository now routes work without anyone creating a label by hand.** Routing fell back to a label named after the repository, and nothing created that label — so a newly connected repository could sit receiving nothing until someone made it manually. Where your workspace has a single team we now route on that team directly. Workspaces with several teams are unchanged and keep using the label, which we say at the time.
- **Authenticating no longer tells you to restart.** Configuration has been picked up automatically for a long time; the instruction was wrong, and following it interrupted whatever the agent was working on.
...
### Changed
- **A review thread now says whose move it is, in plain words.** When the agent finished a turn and it was the reviewer's turn to act, the thread was labelled "Stopped responding" — which reads like a crash rather than a handover. Each end of a turn now says what state the work is in and who moves next: held for review, released to the client, or waiting on the client's answer. ([PON-221](https://linear.app/ponte-digital/issue/PON-221))
- **You can ask the client for something in plain language.** `ask client: <question>` still works, but so do "can you ask the client for the logo files" and "check with the client on the column order" — no keyword to remember. The word *client* is still required, and deliberately: it is the one action that reaches the client's own thread, so a phrase like "ask the client-facing team to review the copy" is left on the review thread rather than guessed at. ([PON-221](https://linear.app/ponte-digital/issue/PON-221))
- **The preview arrives as a link you can click.** It used to be pasted as a raw address — with an access value appended, long enough that the visible text was mostly secret, and reliant on the address being turned into a link automatically. It is now a short labelled link, with the sign-in accounts beside it. Where the preview cannot actually be opened, the link no longer claims otherwise. ([PON-221](https://linear.app/ponte-digital/issue/PON-221))
- **Times on a review thread now measure the review, not the client's issue.** A thread minutes old could show "10 hours ago", because the only age on the board belonged to the client's issue — which had often been open all night while its scope was agreed. Every time shown is now measured from the review's own life, and reads as an age ("in verification · for 2h 5m") rather than a raw timestamp. ([PON-221](https://linear.app/ponte-digital/issue/PON-221))
- **A review thread's first message no longer describes a moment that has passed.** Opening a thread the instant work began, it announced that there was nothing to review and nothing had been sent — true for a second, and read as "this is dead" rather than "this is starting". It now says the work is underway, what will appear when it is ready, and that nothing reaches the client until it is released. ([PON-221](https://linear.app/ponte-digital/issue/PON-221))

### Added
- **Onboarding now tells you what your repository needs before you pay, not after.** We read the repository and work out whether every pull request builds a preview, which host it uses, which database it runs on, and which environment variables the app actually reads — including the ones no file in the repository documents. You get a short, specific list of what we still need instead of a questionnaire about your own setup. Where something would stop us reviewing your work — a preview we cannot open, or one running against your live database — we say so plainly before a lane starts rather than at the first delivery.
- **Work is never reviewed against your production data.** If we have not confirmed that your preview uses a database separate from your live site, the preview is withheld from review entirely and the work is checked from the code instead, with anything we could not exercise stated on delivery. Unconfirmed is treated as unsafe on purpose.
- **Your preview link can now be opened by people without an account on your hosting provider.** Previews are protected by default on most paid hosting plans, which meant the link we sent you showed a login page for an account you do not have. Supply the bypass secret from your own project and the links we send open directly — while the preview stays protected against anyone who does not have that link. Rotating the secret takes effect immediately, and the value is never written into your repository, never logged, and stripped from anything the agent writes.
- **Review work now starts with the running preview, not a diff.** Each piece of work awaiting review carries a link to its deployed preview, with the build state shown honestly: while a build is running it says so and offers nothing to click, a failed build links its logs instead of a broken page, and a repository with no preview environment says that rather than leaving a silent gap. The link is read from the deployment itself, so it follows each new commit. Alongside it, the list of files the change actually touched — read from the pull request, not from what the agent said it did.
- **You can now read what the agent actually did.** Working narration — its thinking, the commands it ran, the files it edited — is kept off the client'"'"'s issue, but it used to be discarded rather than kept, so nobody could read it afterwards. It now lands in full on the internal review thread, where anyone on the team can follow exactly how a change was made.
- **The operator cockpit now says what to pick up next, and lets you claim it.** Work waiting on a reviewer is ranked across clients — each client keeps their own queue, the cycle only decides whose turn it is — and every waiting item says where it sits, with the next one up marked plainly. Assigning yourself claims a piece of work, and claiming is now real: the cockpit no longer overwrites the assignee on every update, so a claim sticks and two reviewers cannot pick up the same thing. Reviewers are notified as subscribers instead, so nothing is lost. ([PON-211](https://linear.app/ponte-digital/issue/PON-211))
- **A pull request now stays a draft until the work is actually released.** Draft state is what tells you a change is not finished yet, and it used to depend on the agent choosing to leave it alone — which it did not always do, so a finished-looking PR could appear on your repository before anyone had reviewed it. Now the agent is told plainly that marking a PR ready is not its decision, and the platform re-checks when work reaches review: a PR that was marked ready is put back into draft, and if that fails the operator is told, loudly, that you can currently see it. Releasing the work is still the only thing that marks it ready. ([PON-208](https://linear.app/ponte-digital/issue/PON-208))
- **The operator cockpit is now a place to work, not just a button to press.** A mirror used to accept two words — `approve` or `reject:` — and answer anything else with "discussion belongs on the client's issue". Now an ordinary instruction on the mirror continues the work: the same conversation, the same worktree, the same branch and the same pull request, with the full working narration in the mirror thread. Throughout, the client sees nothing — no activity, no status change, and the PR stays a draft — because the session posts only to the cockpit and the client's workspace is not reachable from it. Three more verbs: `mine` takes the branch (the mirror then shows the clone URL, branch, PR and a ready-to-paste checkout command), `back to you: <notes>` hands it back and the agent rebases onto your commits before continuing, and `ask client: <question>` is the one thing that can reach the client mid-review. `approve: <notes>` still releases, from any state and whoever wrote the code. The work is billed to the client's own credential, not the cockpit's, and destructive git (force push, hard reset) is denied for the whole session. Every mirror action now requires a configured reviewer. ([PON-208](https://linear.app/ponte-digital/issue/PON-208))
- **The log now says which GitHub credential served each operation.** Every installation token minted for a repository writes one line — `[event:github_token_minted] {"owner":…,"repo":…,"installationId":…,"purpose":"clone|fetch|push|github-api"}` — and any operation that falls back to the ambient `GITHUB_TOKEN`, or runs with no App credential at all, writes a distinctly named `[event:github_token_ambient_fallback]` line saying why. Clone, session-time fetch and API calls all report identically. Token values are never written. Previously the credential path was silent, so "which credential did this use" could only be inferred from whether the operation happened to succeed — which a personal credential on the machine can also explain. ([PON-176](https://linear.app/ponte-digital/issue/PON-176), [#41](https://github.com/ItzHarold/cyrus/pull/41))
- **The operator cockpit now orders work fairly across companies, and review is no longer a one-person job.** Cockpit mirrors are kept in round-robin order — each company's items interleave (A1, B1, C1, A2 …), with work waiting on the operator sorted first — so no tenant monopolizes attention. And `cockpit.reviewers` accepts a list: any member can approve or reject delivery, the first entry is the default assignee, and `cockpit.assignments` can route a specific company's reviews to a specific reviewer. Existing single-`assigneeId` configs behave exactly as before. ([PON-173](https://linear.app/ponte-digital/issue/PON-173))
- **Mid-work questions now pause the work instead of stalling the queue.** When the agent is genuinely missing something only the client can supply — a credential name, a copy decision — it asks once, on the client's issue, framed around the deliverable ("To finish the export you'll receive, I need: …"), with everything it needs in one question. While the answer is pending the client's other queued issues proceed, the operator cockpit shows the issue as `needs-info`, and the wait survives restarts. When the answer arrives the same session resumes exactly where it stopped, with the answer as context. Internal or technical questions are never asked — those are the agent's to resolve. ([PON-172](https://linear.app/ponte-digital/issue/PON-172))
- **Delivery now speaks with the operator's voice too.** Approving completed work accepts notes — `approve: tested the export path end to end` — and the client's summary arrives as a finished delivery: the outcome, a "see it working" preview link when one exists, the PR to merge, and the operator's notes under *Notes from our review*. Bare `approve` still works; `reject: <feedback>` is unchanged and still tells the client nothing. Notes are checked against the client-language policy before anything happens: a note containing an internal term refuses the whole delivery with a rephrase ask — the PR stays draft, nothing posts. Nothing auto-releases, exactly as before. ([PON-171](https://linear.app/ponte-digital/issue/PON-171))
- **On approval, the operator cockpit's mirror becomes a working brief.** The moment a client approves scope, their mirror issue carries everything needed to review the work later without opening the client's workspace: the exact scope text the client approved, when they approved it and after how many revisions, the agent's internal reading, and — as they become known — the PR links. The brief stays on the mirror through every later state change. Operator-side only; nothing on the client's issue changes. ([PON-170](https://linear.app/ponte-digital/issue/PON-170))
- **Scope confirmations now describe the deliverable, not the build.** The comment a client approves states what they will receive — the outcome in their terms, where they will see it working, and what they will get to merge — plus anything the agent had to interpret about the outcome, so they can correct it. Implementation detail (files, approach, technical steps) no longer appears on the client's issue at all: the agent records that internal reading separately for the operator, before the client is asked, and it shows up in the operator cockpit's mirror of the issue — so the operator sees how the work will be done while the client decides whether it should be. A revised scope updates both sides. ([PON-169](https://linear.app/ponte-digital/issue/PON-169))
- **What the client reads now stays in the client's language.** Everything posted on a client's issue or landing in their repository is governed by a content policy: no internal tooling names, no internal file paths, no model names, no running narration — results, not machinery. The policy is enforced three ways: every built-in message template is checked by an automated test (a violation is a build failure, not a production incident), every session carries the rule in its instructions, and outbound text is scanned at post time — unambiguous slips are rewritten (never silently: every rewrite is journaled) and everything else is logged for the operator. References to the client's own branches and repository files are untouched. ([PON-168](https://linear.app/ponte-digital/issue/PON-168))
- **Completed work now waits for the operator's approval before the client is told it exists.** In a verification-gated workspace (`verifyBeforeDelivery`, on by default; explicit `false` opts out) a session's completion summary is held instead of posted — progress activity is unchanged, only the "this is done" moment is gated. The cockpit mirror flips to `in-verification`, gets assigned to the operator (`cockpit.assigneeId`) with the held summary and the draft PR links, and one @mention on the mirror does the rest: `approve` marks the PR ready for review and then posts the client summary (in that order — the client is never pointed at a draft); `reject: <feedback>` sends the work back to the agent with the feedback, and the client is told nothing. Nothing ever auto-releases: after `verificationEscalation.remindAfterHours` (default 4) the operator gets a second, louder notification, and after `delayNoteAfterHours` (default 24) the client's issue gets one honest delay note — never the unverified work. Held deliveries survive restarts, are listed at `GET /admin/verification` (localhost only), and a replayed approval cannot deliver twice. ([PON-152](https://linear.app/ponte-digital/issue/PON-152))
- **An operator cockpit: every delegated issue across every connected workspace, mirrored into one Linear project.** Configure `cockpit: { linearWorkspaceId, workspaceName, teamId, projectId? }` and each delegated issue appears as a mirror issue — titled `[DVV-12] <title>`, labeled with its state (`awaiting-scope-confirm`, `queued`, `active`), closed when the work ends — in the team and project you name. The mirror is a derived view only: the client's issue stays authoritative, nothing reads the mirror back, and a restart reconciles it against reality (including adopting mirrors it finds in Linear rather than duplicating them). The declared `workspaceName` must match the configured workspace's name or mirroring refuses loudly — a copied-wrong workspace id cannot write into the wrong workspace. Mentions are conversations, not delegations, and are never mirrored. Absent config = off, including on hot-reload. ([PON-151](https://linear.app/ponte-digital/issue/PON-151))
- **A delegated issue now gets its scope confirmed before any code is written.** The agent reads the issue and the repository, posts its reading — what it will do, what it will not touch, anything it had to interpret — and asks a structured question: Approve scope, Revise scope, or Cancel. Nothing is implemented until "Approve scope" is chosen; a revision request produces a revised reading, not code; free-text replies never count as approval. The approval belongs to the issue and survives restarts, so a confirmed issue is never asked twice — and the approval moment is recorded, which is when the delivery clock starts. While a session waits on confirmation it hands back its lane slot, so other queued issues proceed. On by default for every workspace; set `scopeConfirmGate: false` on a workspace to opt out. Issues waiting on confirmation are listed at `GET /admin/scope-approvals` (localhost only). ([PON-150](https://linear.app/ponte-digital/issue/PON-150))
...
- Oversized issues are now split before any code is written. An issue that clearly bundles several unrelated features, an "and also" chain, or a migration plus the features built on it gets a proposal instead of an implementation: a question listing the sub-issues it would create, each with a title and a one-line scope. Approving it creates the sub-issues under the original in execution order, hands the first one to the agent, and posts a response naming which one is underway and how to start the rest. The original stays open for you to close. The bar is deliberately high — one feature across many files, one deep bug, one large refactor, and anything borderline all proceed with no comment on scope. ([PON-113](https://linear.app/pontedigital/issue/PON-113))

- The agent now states the reading it is building to when an issue could be understood more than one way. Before starting, it checks whether any instruction has a second plausible meaning that would change what gets built — and if so, records which reading it chose and why, then proceeds. It never asks and never waits, so a wrong interpretation is visible in the first minute instead of at review. Issues with no genuine ambiguity start silently, as before. ([PON-114](https://linear.app/pontedigital/issue/PON-114))

### Fixed
- **Working on a piece of review work no longer requires being the approver.** Anyone on the team can open it, read everything, and talk to the agent about it. Only the two things the client experiences — releasing the work, and asking the client a question — stay restricted to the configured reviewer.
- **Work the client cancelled no longer reappears, or reads as delivered.** A cancelled or completed issue whose notification arrived while the service was down left its cockpit entry sitting open in a live-looking state indefinitely — surviving even being closed by hand, because the restart put it back. Startup now checks with the tracker whether each open piece of work is genuinely still open. And an abandoned issue closes as cancelled rather than as delivered, so the board does not show work you dropped as work we shipped.
- **Handing a piece of review work to the agent used to do nothing.** Delegating it — the most natural way to pick something up — carried no message, so it was read as an empty instruction and answered with silence. It now tells you what the work is, which branch and pull request it is on, and what you can say next, without starting a billable turn.
- **Mentioning the wrong agent gave a confusing prompt about repositories.** Where several agents share a workspace, only one of them can see a given piece of review work; the others used to ask which repository to use and offer their own. They now say plainly that the work belongs to another agent, and every review item names the one that can answer it.
- **The agent was given contradictory instructions when working with a reviewer.** It was told both to write for the client — no technical detail, no running commentary — and to talk to the reviewer like an engineer, at the same time. On a reviewer thread it now gets only the second.

### Changed
- **Each workspace now declares which Anthropic credential its sessions run on**, and a workspace that has not declared one is refused by name rather than quietly using whatever the machine happens to hold. Set `anthropicAuth` on a workspace to `{"mode":"apiKey","apiKey":"…"}` or, to run it on the machine's subscription deliberately, `{"mode":"subscription"}`. Existing installs must add this before upgrading — an undeclared workspace stops starting sessions.
- **Workspaces are now serialized by default**: one active session at a time, with the rest queued in order. Previously this was opt-in. Set `laneSerialization: false` to keep running sessions concurrently, or `laneConcurrency: N` to allow N at once.
- **GitHub access is now resolved per repository** rather than from one machine-wide installation id. `GITHUB_APP_INSTALLATION_ID` is no longer used and can be removed. A repository your GitHub App does not cover now fails with a message naming that repository and what was being attempted, instead of being served with a token belonging to a different organisation.
- Authorising a Linear workspace no longer requires stopping the agent. `cyrus self-auth-linear` used to bind the port the service listens on to catch the OAuth redirect, so onboarding a workspace meant an outage for every other workspace on the same host. The running service now catches the redirect itself, and the terminal shows the authorize URL, waits up to 15 minutes with a progress line each minute, and finishes without a restart. If nothing is running — a fresh install — the old standalone flow still applies. ([PON-126](https://linear.app/ponte-digital/issue/PON-126), [#13](https://github.com/ItzHarold/cyrus/pull/13))
- A session waiting on your answer no longer blocks the rest of your queue. Questions have no deadline, so a session parked on one used to hold a serialized workspace's single slot until someone replied — an approval left overnight stalled every other issue behind it. The slot is now handed back while the session waits and reclaimed when you answer. Only one issue is ever worked at a time, as before. ([PON-113](https://linear.app/pontedigital/issue/PON-113))
...
### Fixed
- Repository package names are no longer mistaken for model names by the client-language policy. Names of the form `claude-<something>` that are packages rather than models — including the agent SDK dependency — were being rewritten to "the model" wherever they appeared, garbling file paths like `packages/claude-runner` in reports. They now pass through untouched; real model names are still redacted exactly as before, and a bare mention of a model family is still flagged. ([PON-186](https://linear.app/ponte-digital/issue/PON-186), [#42](https://github.com/ItzHarold/cyrus/pull/42))
- **The model pin now checks every model in a session, not just the biggest one.** Any model that isn't the pinned one or a known internal helper fails the session and names the offender with its share of the work — previously an unpinned model could quietly carry up to half a session. ([PON-147](https://linear.app/ponte-digital/issue/PON-147))
- Credential files written during configuration updates — including their backups — are now always private to the owner (0600), corrected on every write rather than inherited by luck, and backups are bounded to the five most recent instead of accumulating indefinitely. ([PON-148](https://linear.app/ponte-digital/issue/PON-148))
- **A busy API no longer kills sessions silently.** A session that dies at startup on a transient provider failure (overloaded, rate-limited, connection reset) now retries itself with increasing, jittered delays — several attempts over a few minutes, re-taking its queue slot politely. Auth and billing failures still fail immediately. If retries run out, the client is told plainly on the issue instead of watching an acknowledged session go quiet forever. ([PON-138](https://linear.app/ponte-digital/issue/PON-138))
- **Idle workspaces no longer die silently.** A liveness ping now checks every connected Linear workspace every few minutes and — because it runs through the same client that refreshes tokens on failure — an expired token is refreshed on the spot instead of merely being reported dead. A workspace whose credentials are truly gone (refresh token dead too) is deactivated loudly through the existing revocation path within one interval, instead of sitting unreachable until a client's session failed. `cyrus check-tokens` and `cyrus refresh-token` no longer hang on completion, and `refresh-token` now tries the silent refresh first — the browser flow is only for a genuinely dead refresh token, and only when run interactively. ([PON-136](https://linear.app/ponte-digital/issue/PON-136))
- **Path hygiene and quietness are now separate dials.** Internal server paths are scrubbed from every client-visible message unconditionally — narrating and quiet workspaces alike; paths inside the working copy appear repo-relative. Whether a workspace's session threads show working narration is now its own per-workspace `clientQuiet` setting, independent of the review gates: existing workspaces keep exactly their current behavior until the flag is set. ([PON-182](https://linear.app/ponte-digital/issue/PON-182))
- **The client's activity stream no longer shows the working diary.** On workspaces running the client-flow gates, the mid-session narration — every tool call, file path, and internal step — is suppressed: the client-visible stream is the acknowledgment, a single "On it" status per work burst, questions that need their answer, and the final result. Anything that still posts is scrubbed: file paths appear repo-relative (`SUPPORT.md`, never a server path). Non-gated workspaces are unchanged. Also fixed: a corner of the path-scrubbing that a dotted directory name could slip through. ([PON-179](https://linear.app/ponte-digital/issue/PON-179))
- Links in client-facing messages are no longer rewritten by the client-language policy. A preview URL that embeds the agent's branch name (as Vercel preview hosts do) was being redacted into a broken link; URLs are now exempt as functional pointers — the policy still applies to everything around them. ([PON-168](https://linear.app/ponte-digital/issue/PON-168))

- **A resumed session now lands in a real checkout — or does not start.** Resuming (a scope approval, a reply after a restart) previously trusted whatever workspace the session had recorded, even a deleted directory or an empty fallback folder — observed in production as a session resumed into a missing worktree. Resume now validates the workspace, re-creates it through the same authenticated path a fresh session uses (all of the session's repositories, on the base branches recorded at creation), re-validates the result, and treats an unpreparable working copy as a visible, terminal failure instead of something to run in. ([PON-164](https://linear.app/ponte-digital/issue/PON-164))
- **A refused worktree now refuses the whole session.** The stale-code refusal below could be absorbed by a fallback that handed the session an empty directory as its workspace — observed in production: the refusal fired correctly and the session started anyway, in a worktree containing nothing. The refusal is now terminal: the session never starts, the issue shows an error activity naming the repository, and the workspace lane frees for the next issue. ([PON-161](https://linear.app/ponte-digital/issue/PON-161))
- **Session worktrees now authenticate to private GitHub repositories.** The per-repository GitHub App credential existed for onboarding clones but never reached the session-time `git fetch`/`ls-remote` — on a private repository every session start fell to interactive auth and failed. The same per-repo token now covers every worktree path; remote URLs stay tokenless. ([PON-162](https://linear.app/ponte-digital/issue/PON-162))
- **A session no longer continues silently against stale code when it cannot reach the remote.** `git fetch` failures were caught and the session carried on with whatever was last cloned, reporting success — so a run could produce a diff against code that had already moved on, with only a warning nobody reads. A repository with a remote it cannot reach now fails, naming the repository. A repository with no remote configured is unaffected.
- `cyrus self-add-repo` now clones private repositories using your GitHub App instead of stopping at a username prompt. Previously the clone ran with no credentials at all: on a machine with `gh auth login` already set up it silently borrowed that personal account, and on a clean server it hung waiting for input that never came. The token is minted for that repository's own installation, used for the single clone, and never written to `.git/config` or the saved remote. If no installation covers the repository, the clone stops and says which one is missing rather than reaching for another account's access. ([PON-145](https://linear.app/ponte-digital/issue/PON-145))
- `cyrus self-auth-linear` no longer waits fifteen minutes when something other than the agent is listening on its port. A response that does not look like the agent's own is now treated as "no agent here" and the standalone browser flow starts immediately. ([PON-145](https://linear.app/ponte-digital/issue/PON-145))
- An OAuth approval that takes more than a few minutes no longer strands the authorisation. The wait was five minutes while the service held the pending code for ten, so an approval landing in between left a valid code that nothing could claim — retrying issued a fresh request, and the whole authorisation had to be redone. Both windows are now 15 minutes, and if it does time out the error prints exactly how to collect a code that arrived late. ([PON-126](https://linear.app/ponte-digital/issue/PON-126), [#13](https://github.com/ItzHarold/cyrus/pull/13))
