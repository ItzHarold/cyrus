...
### Fixed
- **Connecting a repository now routes work without anyone creating a label by hand.** Routing fell back to a label named after the repository — and nothing created it, so a newly connected repository could sit there receiving nothing until someone made the label manually. Where your workspace has a single team, we now route on that team directly. Workspaces with several teams are unchanged and still use the label, which we tell you at the time.
- **Two things changing your configuration at once can no longer lose one of them.** Connecting a repository, authorising a workspace, and the routine refresh of access tokens all write the same configuration file. Done at the same moment, one could overwrite the other and quietly drop a connected workspace. Writes are now exclusive and atomic: the file is never half-written, and a change cannot be lost to a simultaneous one.
