...
### Fixed
- Repository package names are no longer mistaken for model names by the client-language policy. Names of the form `claude-<something>` that are packages rather than models — including the agent SDK dependency — were being rewritten to "the model" wherever they appeared, garbling file paths like `packages/claude-runner` in reports. They now pass through untouched; real model names are still redacted exactly as before, and a bare mention of a model family is still flagged. ([PON-186](https://linear.app/ponte-digital/issue/PON-186))
