...
### Fixed
- **Stopping a run from the cockpit no longer closes the work as done.** A mirror run that is stopped, crashes, or is interrupted by a restart goes back to the queue with a note; delegating the mirror again picks it up on the same branch. Previously the mirror closed as delivered with the work half-built.
- **A plain instruction on a cockpit run no longer changes where the delivery goes.** After one iteration, a later rewrite request could regenerate on the client's own thread; it now stays on the cockpit.
