...
### Fixed
- **The agent's Linear credential no longer appears in the command line of the session process.** The tool-server configuration, which carries that credential, is now handed to the session as a private file that exists only while the session runs.
