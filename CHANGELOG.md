...
- Issues that are missing something essential now get one question before any code is written, instead of a guess. When an issue does not say what to build, where it lives, or what done looks like — and the answer isn't in the repo — the agent asks once, with up to three concrete questions and the answer it would otherwise assume beside each. Approving the defaults starts the work immediately; you can also just reply in your own words. Clear issues start with no questions and no comment on clarity. ([PON-114](https://linear.app/pontedigital/issue/PON-114))
